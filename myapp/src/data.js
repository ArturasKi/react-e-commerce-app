import summer from "./Images/Categories/summer.png";
import autumn from "./Images/Categories/autumn.png";
import winter from "./Images/Categories/winter.png";
import pr_shoes1 from "./Images/Products/Mens/Shoes/pr_shoes1.png";
import pr_shoes2 from "./Images/Products/Mens/Shoes/pr_shoes2.png";
import pr_shoes3 from "./Images/Products/Mens/Shoes/pr_shoes3.png";
import pr_jeans1 from "./Images/Products/Mens/Pants/pr_jeans1.png";
import pr_jeans2 from "./Images/Products/Mens/Pants/pr_jeans2.png";
import pr_jeans3 from "./Images/Products/Mens/Pants/pr_jeans3.png";
import pr_shorts from "./Images/Products/Mens/Pants/pr_shorts.png";
import pr_shirt1 from "./Images/Products/Mens/Shirts/pr_shirt1.png";
import pr_shirt2 from "./Images/Products/Mens/Shirts/pr_shirt2.png";
import pr_shirt3 from "./Images/Products/Mens/Shirts/pr_shirt3.png";
import pr_tshirt1 from "./Images/Products/Mens/Tshirts/pr_tshirt1.png";
import pr_tshirt2 from "./Images/Products/Mens/Tshirts/pr_tshirt2.png";
import pr_tshirt3 from "./Images/Products/Mens/Tshirts/pr_tshirt3.png";
import pr_cap1 from "./Images/Products/Mens/Accessories/pr_cap1.png";
import pr_cap2 from "./Images/Products/Mens/Accessories/pr_cap2.png";
import pr_cap3 from "./Images/Products/Mens/Accessories/pr_cap3.png";
import pr_cap4 from "./Images/Products/Mens/Accessories/pr_cap4.png";
import pr_cap5 from "./Images/Products/Mens/Accessories/pr_cap5.png";
import pr_sweater1 from "./Images/Products/Mens/Sweaters/pr_sweater1.png";
import pr_sweater2 from "./Images/Products/Mens/Sweaters/pr_sweater2.png";
import pr_sweater3 from "./Images/Products/Mens/Sweaters/pr_sweater3.png";
import pr_jacket1 from "./Images/Products/Mens/Jackets/pr_jacket1.png";
import pr_jacket2 from "./Images/Products/Mens/Jackets/pr_jacket2.png";
import pr_jacket3 from "./Images/Products/Mens/Jackets/pr_jacket3.png";
import wpr_acc_1 from "./Images/Products/Womens/Accessories/wpr_acc_1.png";
import wpr_acc_2 from "./Images/Products/Womens/Accessories/wpr_acc_2.png";
import wpr_acc_3 from "./Images/Products/Womens/Accessories/wpr_acc_3.png";
import wpr_acc_4 from "./Images/Products/Womens/Accessories/wpr_acc_4.png";
import wpr_acc_5 from "./Images/Products/Womens/Accessories/wpr_acc_5.png";
import wpr_dress_1 from "./Images/Products/Womens/Dresses/wpr_dress_1.png";
import wpr_dress_2 from "./Images/Products/Womens/Dresses/wpr_dress_2.png";
import wpr_dress_3 from "./Images/Products/Womens/Dresses/wpr_dress_3.png";
import wpr_jacket_1 from "./Images/Products/Womens/Jackets/wpr_jacket_1.png";
import wpr_jacket_2 from "./Images/Products/Womens/Jackets/wpr_jacket_2.png";
import wpr_jacket_3 from "./Images/Products/Womens/Jackets/wpr_jacket_3.png";
import wpr_jeans_1 from "./Images/Products/Womens/Jeans/wpr_jeans_1.png";
import wpr_jeans_2 from "./Images/Products/Womens/Jeans/wpr_jeans_2.png";
import wpr_jeans_3 from "./Images/Products/Womens/Jeans/wpr_jeans_3.png";
import wpr_shoes_1 from "./Images/Products/Womens/Shoes/wpr_shoes_1.png";
import wpr_shoes_2 from "./Images/Products/Womens/Shoes/wpr_shoes_2.png";
import wpr_shoes_3 from "./Images/Products/Womens/Shoes/wpr_shoes_3.png";
import wpr_skirt_1 from "./Images/Products/Womens/Skirts/wpr_skirt_1.png";
import wpr_skirt_2 from "./Images/Products/Womens/Skirts/wpr_skirt_2.png";
import wpr_sweater_1 from "./Images/Products/Womens/Sweaters/wpr_sweater_1.png";
import wpr_sweater_2 from "./Images/Products/Womens/Sweaters/wpr_sweater_2.png";
import wpr_sweater_3 from "./Images/Products/Womens/Sweaters/wpr_sweater_3.png";
import wpr_sweater_4 from "./Images/Products/Womens/Sweaters/wpr_sweater_4.png";
import wpr_sweater_5 from "./Images/Products/Womens/Sweaters/wpr_sweater_5.png";
import wpr_tee_1 from "./Images/Products/Womens/Tees/wpr_tee_1.png";
import wpr_tee_2 from "./Images/Products/Womens/Tees/wpr_tee_2.png";
import wpr_tee_4 from "./Images/Products/Womens/Tees/wpr_tee_4.png";
import wpr_tee_5 from "./Images/Products/Womens/Tees/wpr_tee_5.png";

export const sliderData = [
  {
    id: 1,
    img: summer,
    title: "SUMMER SALE",
    desc: "GET OVER 70% OFF FOR SUMMER COLLETION CLOTHES!",
    bg: "#fffaaa",
  },
  {
    id: 2,
    img: autumn,
    title: "AUTUMN COLLECTION",
    desc: "SEE WHAT'S NEW IN AN UPCOMING AUTUMN COLLECTION.",
    bg: "#edaeda",
  },
  {
    id: 3,
    img: winter,
    title: "WINTER IS COMING...",
    desc: "COMING SOON.",
    bg: "#adeeda",
  },
];

export const categories = [
  {
    id: 1,
    img: "https://cdn.shopify.com/s/files/1/0085/7450/0979/products/BC-01-TAPERED-CHINO-11-oz-military-selvedge-denim-fit-4_0b2f65e5-be77-4eda-b538-0420d3bc37df_1000x1000.jpg?v=1647438436",
    title: " BEST SELVEDGE DENIM QUALITY!",
  },
  {
    id: 2,
    img: "https://media.endclothing.com/media/f_auto,q_auto:eco/prodmedia/media/catalog/product/1/4/14-01-2021_MO_8138_m6_1.jpg",
    title: "LONG LASTING LEATHER SHOES!",
  },
  {
    id: 3,
    img: "https://d13qso5xfejx18.cloudfront.net/product-media/6INZ/1000/1000/19H216365-0066Front211.jpg",
    title: "CREATE YOUR RUGGED STYLE!",
  },
];

export const products = [
  {
    id: 1,
    title: "Red Wing Classic Moc",
    category: "shoes",
    price: "299.00 EUR",
    img: pr_shoes1,
    color: ["black", "brown", "olive", "burlywood"],
    sale: 0,
  },
  {
    id: 2,
    title: "DenimDudes selvedge jeans",
    category: "pants",
    price: "129.00 EUR",
    img: pr_jeans1,
    color: ["blue", "navy", "black", "lightblue"],
    sale: 0,
  },
  {
    id: 3,
    title: "Deus cap",
    category: "accessories",
    price: "19.00 EUR",
    img: pr_cap1,
    color: ["black", "blue", "gray"],
    sale: 0,
  },
  {
    id: 4,
    title: "Deus sweater",
    category: "sweaters",
    price: "69.99 EUR",
    img: pr_sweater1,
    color: ["black", "white", "gray"],
    sale: 1,
  },
  {
    id: 5,
    title: "Deus autumn jacket",
    category: "jackets",
    price: "155.99 EUR",
    img: pr_jacket1,
    color: ["black", "orange", "brown"],
    sale: 0,
  },
  {
    id: 6,
    title: "Swimming shorts",
    category: "pants",
    price: "49.99 EUR",
    img: pr_shorts,
    color: ["navy", "white", "rosybrown"],
    sale: 1,
  },
  {
    id: 7,
    title: "Summer shirt",
    category: "shirts",
    price: "35.99 EUR",
    img: pr_shirt1,
    color: ["green", "coral", "olive"],
    sale: 1,
  },
  {
    id: 8,
    title: "Deus Boulevard t-shirt",
    category: "tshirts",
    price: "24.99 EUR",
    img: pr_tshirt1,
    color: ["blue", "black", "darkolivegreen", "goldenrod"],
    sale: 1,
  },
  {
    id: 9,
    title: "Red Wing Iron Rangers",
    category: "shoes",
    price: "329.00 EUR",
    img: pr_shoes2,
    color: ["black", "brown", "olive"],
    sale: 0,
  },
  {
    id: 10,
    title: "Red Wing Blacksmith",
    category: "shoes",
    price: "319.00 EUR",
    img: pr_shoes3,
    color: ["black", "brown", "olive", "chocolate", "slategrey"],
    sale: 0,
  },
  {
    id: 11,
    title: "Edwin selvedge jeans",
    category: "pants",
    price: "100.00 EUR",
    img: pr_jeans2,
    color: ["black", "navy", "slategrey"],
    sale: 0,
  },
  {
    id: 12,
    title: "Denim selvedge jeans",
    category: "pants",
    price: "89.99 EUR",
    img: pr_jeans3,
    color: ["black", "navy", "midnightblue"],
    sale: 0,
  },
  {
    id: 13,
    title: "Levis vintage jacket",
    category: "jackets",
    price: "120.00 EUR",
    img: pr_jacket2,
    color: ["black", "midnightblue", "whitesmoke"],
    sale: 0,
  },
  {
    id: 14,
    title: "Leather jacket",
    category: "jackets",
    price: "150.00 EUR",
    img: pr_jacket3,
    color: ["black", "brown"],
    sale: 0,
  },
  {
    id: 15,
    title: "Flannel shirt",
    category: "shirts",
    price: "49.00 EUR",
    img: pr_shirt2,
    color: ["brown", "olive"],
    sale: 0,
  },
  {
    id: 16,
    title: "Summer shirt",
    category: "shirts",
    price: "33.00 EUR",
    img: pr_shirt3,
    color: ["mediumaquamarine"],
    sale: 1,
  },
  {
    id: 17,
    title: "Light autumn sweater",
    category: "sweaters",
    price: "59.99 EUR",
    img: pr_sweater2,
    color: ["darkblue", "darkgreen", "darkgray"],
    sale: 0,
  },
  {
    id: 18,
    title: "Peanuts hoodie",
    category: "sweaters",
    price: "49.99 EUR",
    img: pr_sweater3,
    color: ["orange", "darkolivegreen"],
    sale: 0,
  },
  {
    id: 19,
    title: "Yoda t-shirt",
    category: "tshirts",
    price: "29.99 EUR",
    img: pr_tshirt2,
    color: ["darkblue", "black", "orange"],
    sale: 1,
  },
  {
    id: 20,
    title: "Patagonia t-shirt",
    category: "tshirts",
    price: "32.99 EUR",
    img: pr_tshirt3,
    color: ["black", "white", "chocolate", "aquamarine", "thistle"],
    sale: 1,
  },
  {
    id: 21,
    title: "Patagonia cap",
    category: "accessories",
    price: "32.99 EUR",
    img: pr_cap2,
    color: ["black", "navy", "olive", "darkgrey"],
    sale: 0,
  },
  {
    id: 22,
    title: "Patagonia cap",
    category: "accessories",
    price: "32.99 EUR",
    img: pr_cap3,
    color: ["black", "navy", "olive", "darkgrey"],
    sale: 0,
  },
  {
    id: 23,
    title: "Deus trucker cap",
    category: "accessories",
    price: "32.99 EUR",
    img: pr_cap4,
    color: ["black", "navy", "olive", "darkgrey"],
    sale: 1,
  },
  {
    id: 24,
    title: "Deus beanie",
    category: "accessories",
    price: "32.99 EUR",
    img: pr_cap5,
    color: ["black", "navy", "brown", "darkgrey", "darkgreen"],
    sale: 0,
  },
  {
    id: 51,
    title: "Summer hat",
    category: "accessories",
    price: "29.99 EUR",
    img: wpr_acc_1,
    color: ["black", "beige", "white"],
    sale: 0,
  },
  {
    id: 52,
    title: "Summer hat",
    category: "accessories",
    price: "35.99 EUR",
    img: wpr_acc_2,
    color: ["brown", "beige", "white"],
    sale: 0,
  },
  {
    id: 53,
    title: "Summer hat",
    category: "accessories",
    price: "24.99 EUR",
    img: wpr_acc_3,
    color: ["black"],
    sale: 0,
  },
  {
    id: 54,
    title: "Trucker cap",
    category: "accessories",
    price: "19.99 EUR",
    img: wpr_acc_4,
    color: ["black", "blue", "white"],
    sale: 0,
  },
  {
    id: 55,
    title: "Leather gloves",
    category: "accessories",
    price: "49.99 EUR",
    img: wpr_acc_5,
    color: ["brown", "black"],
    sale: 0,
  },
  {
    id: 56,
    title: "Dress",
    category: "dresses",
    price: "44.99 EUR",
    img: wpr_dress_1,
    color: ["tomato", "darkgreen", "darkkhaki"],
    sale: 0,
  },
  {
    id: 57,
    title: "Dress",
    category: "dresses",
    price: "39.99 EUR",
    img: wpr_dress_2,
    color: ["orangered", "darksalmon", "goldenrod"],
    sale: 1,
  },
  {
    id: 58,
    title: "Dress",
    category: "dresses",
    price: "34.99 EUR",
    img: wpr_dress_3,
    color: ["beige", "goldenrod"],
    sale: 0,
  },
  {
    id: 59,
    title: "Light Jacket",
    category: "jackets",
    price: "159.99 EUR",
    img: wpr_jacket_1,
    color: ["lightseagreen", "beige", "palevioletred"],
    sale: 0,
  },
  {
    id: 60,
    title: "Warm Winter Jacket",
    category: "jackets",
    price: "119.99 EUR",
    img: wpr_jacket_2,
    color: ["crimson", "brown"],
    sale: 0,
  },
  {
    id: 61,
    title: "Helly Hansen Jacket",
    category: "jackets",
    price: "199.99 EUR",
    img: wpr_jacket_3,
    color: ["cadetblue", "darkblue", "darkolivegreen", "darkorchid"],
    sale: 0,
  },
  {
    id: 62,
    title: "Flared Jeans",
    category: "jeans",
    price: "69.99 EUR",
    img: wpr_jeans_1,
    color: ["cadetblue", "blue", "lightblue"],
    sale: 0,
  },
  {
    id: 63,
    title: "Slim Jeans",
    category: "jeans",
    price: "89.99 EUR",
    img: wpr_jeans_2,
    color: ["black", "lightblue", "white"],
    sale: 0,
  },
  {
    id: 64,
    title: "Regular Selvedge Jeans",
    category: "jeans",
    price: "149.99 EUR",
    img: wpr_jeans_3,
    color: ["black", "beige", "lightblue"],
    sale: 0,
  },
  {
    id: 65,
    title: "Red Wing Classic Moc",
    category: "wshoes",
    price: "299.99 EUR",
    img: wpr_shoes_1,
    color: ["pink", "brown", "black", "olive"],
    sale: 0,
  },
  {
    id: 66,
    title: "Red Wing Classic Chelsea",
    category: "wshoes",
    price: "309.99 EUR",
    img: wpr_shoes_2,
    color: ["black", "brown", "burlywood", "darksalmon"],
    sale: 0,
  },
  {
    id: 67,
    title: "Red Wing Clara",
    category: "wshoes",
    price: "339.99 EUR",
    img: wpr_shoes_3,
    color: ["black", "brown", "burlywood"],
    sale: 0,
  },
  {
    id: 68,
    title: "Summer Skirt",
    category: "skirts",
    price: "39.99 EUR",
    img: wpr_skirt_1,
    color: ["darksalmon", "beige", "darkorange"],
    sale: 1,
  },
  {
    id: 69,
    title: "Formal Skirt",
    category: "skirts",
    price: "33.99 EUR",
    img: wpr_skirt_2,
    color: ["black", "darkblue", "white"],
    sale: 0,
  },
  {
    id: 70,
    title: "Wool Sweater",
    category: "sweaters",
    price: "99.99 EUR",
    img: wpr_sweater_1,
    color: ["pink", "coral", "maroon"],
    sale: 0,
  },
  {
    id: 71,
    title: "Light Sweater",
    category: "sweaters",
    price: "29.99 EUR",
    img: wpr_sweater_2,
    color: ["coral", "indianred", "darkgreen", "navy"],
    sale: 0,
  },
  {
    id: 72,
    title: "Wool Sweater",
    category: "sweaters",
    price: "89.99 EUR",
    img: wpr_sweater_3,
    color: ["grey", "beige", "white"],
    sale: 0,
  },
  {
    id: 73,
    title: "Oversized Sweater",
    category: "sweaters",
    price: "39.99 EUR",
    img: wpr_sweater_4,
    color: ["orangered", "darksalmon", "white"],
    sale: 0,
  },
  {
    id: 74,
    title: "High Neck Wool Sweater",
    category: "sweaters",
    price: "119.99 EUR",
    img: wpr_sweater_5,
    color: ["maroon", "brown", "darkgreen"],
    sale: 1,
  },
  {
    id: 75,
    title: "La Sportiva Tee",
    category: "tees",
    price: "12.99 EUR",
    img: wpr_tee_1,
    color: ["green", "pink", "yellow"],
    sale: 0,
  },
  {
    id: 76,
    title: "Ponies Tee",
    category: "tees",
    price: "19.99 EUR",
    img: wpr_tee_2,
    color: ["grey", "black", "darkgreen", "darksalmon"],
    sale: 0,
  },
  {
    id: 77,
    title: "Animal Tee",
    category: "tees",
    price: "39.99 EUR",
    img: wpr_tee_4,
    color: ["navyblue", "darkgreen"],
    sale: 0,
  },
  {
    id: 78,
    title: '"Mes uz Ukraina" Tee',
    category: "tees",
    price: "32.99 EUR",
    img: wpr_tee_5,
    color: ["yellow"],
    sale: 1,
  },
];

export const readings = [
  {
    id: 1,
    img: "https://www.fashiongonerogue.com/wp-content/uploads/2018/09/HM-Fall-2018-Casual-Outfit-Ideas01.jpg",
    date: "Aug 28, 2022",
    title: "What's up autumn?!",
  },
  {
    id: 2,
    img: "https://www.byrdie.com/thmb/aToMiU52_2NZkNXG9zgSSpSqzJw=/800x800/filters:fill(auto,1)/greenjumpsuit-c700f1b32e5c4bd3ad561339dc9a25f2.jpg",
    date: "Aug 20, 2022",
    title: "New dress trends",
  },
  {
    id: 3,
    img: "https://www.wedenim.com/wp-content/uploads/2021/09/SauceZhan-317XX-RAW79-Men-Jeans-Selvedge-Raw-Denim-Jeans-Loose-Wide-Leg-Jeans-Unsanforized-Denim-Jeans.jpg",
    date: "Aug 12, 2022",
    title: "Raw denim diaries #2",
  },
  {
    id: 4,
    img: "https://media.lexception.com/img/products/red-wing/64041-red-wing-bootsironrangercopperroughtough-02-0800-0800.jpg",
    date: "Aug 04, 2022",
    title: "Shoes that always looks great",
  },
  {
    id: 5,
    img: "https://wellmagazineasia.com/wp-content/uploads/2021/01/Sustainable-fashion-in-Hong-Kong-Eco-friendly-brands-fashion-into-the-future-s-800x800.jpg",
    date: "Jul 23, 2022",
    title: "Sustainability",
  },
  {
    id: 6,
    img: "https://cdn.shopify.com/s/files/1/0391/8135/3005/products/strike-gold-strikegold-sg7104-7104-super-slubby-slub-15oz-tapered-statement-statementstore-munich-1_1200x1200.jpg?v=1634714375",
    date: "Jul 15, 2022",
    title: "Raw denim diaries #1",
  },
];
